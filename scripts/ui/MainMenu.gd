extends Control
## Main hub: exact 14-zone 16:9 layout from the supplied blueprint.
## 10 HP, 9 MANA, 11 COIN, 12 GEM, 13 HUNTER, 14 DUNGEON
## 1 SETTINGS, 2 TEAM, 3 SHOP, 4 BATTLE, 5 INDEX, 6 EVOLUTION
## 7 is the open centre/background, 8 is the small FARM panel.
...
const STONE_DARK := Color("#5F4933")
const STONE_MID := Color("#806446")
const STONE_LIGHT := Color("#D2B486")
...
func _background() -> void:
    ...
    shade.color = Color(0.03, 0.02, 0.01, 0.13)
...
func _build_ui() -> void:
    ...
    # 7: open centre. No extra helper sentence; the battle button is self-explanatory.
    var title := _label("CATCH A MONSTER", 30)
    _place(title, Rect2(315, 265, 650, 44))
    layout_root.add_child(title)
    var subtitle := _label("— TOWER DEFENSE —", 18)
    _place(subtitle, Rect2(365, 309, 550, 34))
    layout_root.add_child(subtitle)

    # 10 / 9 / 11 / 12 / 13 / 14. Exact rail arrangement.
    hp_zone = _zone("HP\n0 / 0", ShapeButton.Shape.RIGHT_SLOPE, 17)
    mana_zone = _zone("MANA\n0", ShapeButton.Shape.RIGHT_SLOPE, 15)
    coin_zone = _zone("COIN\n0", ShapeButton.Shape.RECT, 17)
    gem_zone = _zone("GEM\n0", ShapeButton.Shape.LEFT_SLOPE, 17)
    var hunter := _zone("HUNTER", ShapeButton.Shape.QUAD, 16)
    var dungeon := _zone("DUNGEON", ShapeButton.Shape.LEFT_SLOPE, 16)

    _place(hp_zone, Rect2(8, 8, 352, 64))
    _place(mana_zone, Rect2(8, 74, 352, 50))
    _place(coin_zone, Rect2(368, 8, 300, 64))
    _place(gem_zone, Rect2(676, 8, 220, 64))
    _place(hunter, Rect2(904, 8, 170, 104))
    _place(dungeon, Rect2(1082, 8, 190, 104))

    # 8: small FARM panel, not a giant column.
    var farm := _zone("FARM", ShapeButton.Shape.QUAD, 16)
    _place(farm, Rect2(10, 150, 150, 112))
    farm.pressed.connect(func(): _coming_soon("Farm"))

    # 1 / 2 / 3 / 4 / 5 / 6 bottom rail.
    var settings := _zone("SETTINGS", ShapeButton.Shape.QUAD, 14)
    var team := _zone("TEAM", ShapeButton.Shape.QUAD, 16)
    var shop := _zone("SHOP", ShapeButton.Shape.RIGHT_SLOPE, 16)
    var battle := _zone("BATTLE", ShapeButton.Shape.BATTLE, 20)
    var index := _zone("INDEX", ShapeButton.Shape.LEFT_SLOPE, 16)
    var evolution := _zone("EVOLUTION", ShapeButton.Shape.LEFT_SLOPE, 15)

    _place(settings, Rect2(10, 632, 148, 80))
    _place(team, Rect2(164, 632, 150, 80))
    _place(shop, Rect2(320, 632, 150, 80))
    _place(battle, Rect2(470, 614, 300, 98))
    _place(index, Rect2(782, 632, 150, 80))
    _place(evolution, Rect2(944, 632, 328, 80))
...
func _zone(value: String, shape_value: ShapeButton.Shape, font_size := 15) -> ShapeButton:
    ...
    b.hover_fill = Color("#C7A879")
    b.pressed_fill = Color("#96764F")
...
func _label(value: String, font_size := 15) -> Label:
    ...
    l.add_theme_color_override("font_outline_color", Color(0, 0, 0, 0.48))
...
func _build_coming_soon_overlay() -> void:
    ...
    var shade := ColorRect.new()
    shade.color = Color(0.02, 0.015, 0.01, 0.72)
    shade.set_anchors_and_offsets_preset(Control.PRESET_FULL_RECT)
    ...
    card.add_theme_stylebox_override("panel", UITheme.flat(TAN, STONE_DARK, 4, 4))
    ...
    card.add_child(v)
    var title := _label("COMING SOON", 26)
    v.add_child(title)
    coming_label = _label("", 14)
    coming_label.autowrap_mode = TextServer.AUTOWRAP_WORD_SMART
    coming_label.custom_minimum_size = Vector2(380, 70)
...
func _coming_soon(feature_name: String) -> void:
    ...
    if is_instance_valid(coming_label):
        coming_label.text = "%s is coming soon." % feature_name

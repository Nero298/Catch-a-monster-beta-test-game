...
func save_game() -> bool:
	...
	if file == null:
		push_error("[SaveManager] Failed to open temp save: " + str(FileAccess.get_open_error()))
		return false
	...
	if rename_error != OK:
		push_error("[SaveManager] Failed to commit save: " + str(rename_error))
		return false
...
func load_game() -> bool:
	...
	GameManager.balls = data.get("balls", {"basic": 15, "great": 3, "ultra": 0})
	GameManager.ball_levels = data.get("ball_levels", {"basic": 1, "great": 1, "ultra": 1})
	...
	print("[SaveManager] Game loaded successfully (v", data.get("version", 1), ")")
	return true
